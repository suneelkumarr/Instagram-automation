...
class AIService {
    ...
    constructor() {
        this.client = new openai_1.default({
            apiKey: index_js_1.config.openai.apiKey,
            baseURL: index_js_1.config.openai.baseURL,
        });
        this.fallbackClient = new openai_1.default({
            apiKey: index_js_1.config.openai.fallbackApiKey,
            baseURL: index_js_1.config.openai.baseURL,
        });
    }
    /**
    ...
    async generateResponse(request) {
        const { contact, message, brandName, niche, customPrompt, conversationHistory = [], } = request;
        // Build context
        ...
        const models = [index_js_1.config.openai.model, index_js_1.config.openai.fallbackModel].filter(Boolean);
        for (let i = 0; i < models.length; i++) {
            ...
            try {
                ...
                console.error(`OpenAI error (${model}):`, error.message);
     ...
     */
    buildContext(contact, brandName, niche) {
     ...
     */
    extractFields(message, response) {
        const fields = {};
        // Extract email
    ...
    detectIntent(text) {
        ...
        if (lower.includes('when') || lower.includes('delivery') || lower.includes('shipping'))
            return 'shipping_question';
        if (lower.includes('refund') || lower.includes('return') || lower.includes('exchange'))
            return 'support_request';
        if (lower.includes('demo') || lower.includes('try') || lower.includes('free'))
            return 'trial_interest';
        if (lower.includes('call') || lower.includes('schedule') || lower.includes('book'))
     ...
     */
    shouldEscalate(text, sentiment, intent) {
     ...
     */
    async testPrompt(prompt, testMessage) {
        const messages = [
        ...
        for (let i = 0; i < models.length; i++) {
            ...
            try {
                ...
                console.error(`testPrompt error (${model}):`, error.message);
            }
        }
        return '';
    }
